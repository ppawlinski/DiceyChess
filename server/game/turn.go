...
func (t *Turn) Start() {
	roll := t.Dice.Roll()
	leftover := t.State.LeftoverBudget[t.State.ColorToMove]
...
func (t *Turn) SkipIfNecessary() bool {
	// zasada 5 - jeśli gracz nie może wykonać żadnego ruchu
	if t.hasAnyLegalMove() {
		return false
	}

	// budżet przechodzi do następnej tury tego gracza
	t.State.LeftoverBudget[t.State.ColorToMove] += t.State.CurrentBudget

	// zmiana gracza bez resetowania budżetu leftover dla tego gracza
	if t.State.ColorToMove == White {
		t.State.ColorToMove = Black
	} else {
		t.State.ColorToMove = White
